...
// fs_utils.coffee
var hideFilesInDir, unhideFilesInDir;
...
// ---------------------------------------------------------------------------
hideFilesInDir = function(srcDir, destDir, regExp) {
  var destPath, destSubDir, ent, i, len, ref, srcPath, srcSubDir;
  ref = readdirSync(srcDir, {
    withFileTypes: true
  });
  for (i = 0, len = ref.length; i < len; i++) {
    ent = ref[i];
...
// ---------------------------------------------------------------------------
unhideFilesInDir = function(destDir, srcDir) {
  var destPath, destSubDir, ent, i, len, ref, srcPath, srcSubDir;
  ref = readdirSync(destDir, {
    withFileTypes: true
  });
  for (i = 0, len = ref.length; i < len; i++) {
    ent = ref[i];
...
// ---------------------------------------------------------------------------
export var unhideFiles = function(rootDir) {
  var hideDir;
  hideDir = mkpath(rootDir, 'hiddenFiles');
  if (!existsSync(hideDir)) {
    return;
  }
  return unhideFilesInDir(hideDir, rootDir);
};
